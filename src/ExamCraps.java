 ...
 * 
 */
public class ExamCraps
{
	public static final int NUMBER_TO_PLAY = 1000000;
	 ...
	 * <code>numPlays</code> total plays.
	 */
	private int[] winMoves = new int[100];
	...
	public ExamCraps()
	{
		...
		match = new crapsTable(new checkmate(), new checkmate()); // two "fair" Die objects
	...
	public void play(int n)
	{
		...
		{
			if (match.playOneGame(winMoves, lossSteps))
			{
				numberWins++;
			}
		}
	...
	{
		for (int i = 1; i < winMoves.length; i++)
		{
			StdOut.println("(wins,losses) ending at " + i + " roll" + ((i > 1) ? "s" : "") + ": (" + winMoves[i] + ","
					+ lossSteps[i] + ")");
		}

		StdOut.println("\nPlayed " + numberPlays + " games total.");

		StdOut.println("Won " + numberWins + "/" + numberPlays + "==" + 100.0 * (0.0 + numberWins) / numberPlays + "%");
	}
	...
	{
		ExamCraps test = new ExamCraps();

		StdOut.println("Starting simulation of " + NUMBER_TO_PLAY + " games...");

		crapsTable.displayOutput = false; // turn off output to speed up
		// simulation

		test.play(NUMBER_TO_PLAY); // play games of craps

		StdOut.println("Done.");

		test.recordStats(); // report the output
